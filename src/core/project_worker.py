...
from core.utils import generate_plugin_id, create_cmake_file, update_workflow_files, create_gitignore_file, create_readme_from_variables
...
class ProjectWorker(QObject):
    ...
    def run(self):
        ...
        try:
            ...
            self.error.emit(f"Project generation failed: {str(e)}")
            self.finished.emit()
    ...
    def cleanup_git_history(self):
        ...
        if os.path.exists(git_dir):
            try:
                ...
                self.progress.emit(f"Error removing Git directory: {str(e)}")
                raise RuntimeError(f"Failed to remove Git directory: {str(e)}")
